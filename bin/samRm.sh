...
echo "
   samRm [OPTIONS] [-f FILE]  [-s FILEOFNAMES] [-d DATASET]
      ...
      -h print help
      -r only retire file records in SAM, do not delete in dCache
...
while getopts d:f:s:hnr OPT; do
    case $OPT in
        f)
...
if [ "$RMLOC" == "yes" ]; then
    ...
    echo "Will retire $N files in SAM, examples:"
fi
